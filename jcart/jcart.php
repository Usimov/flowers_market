...
foreach($this->get_contents() as $item)	{
	if($item['name']){
		...
		if($ostat[1]){
			...
			if($ostat[0] == 0){$XX = 2;}
			if($ostat[0] !=''){
				$ostat2 =explode("-",$ostat[0]);
				$idcc=explode ("|", $item['id']);
				for($b=0;$b<count($ostat2[0]);$b++){
					if($ostat2[0] == $idcc[1]){
						if($ostat2[1] == ''){$ostat2[1] = 0;}
						if($ostat2[1] == 0){$XX = 2;}
						$ostX[$item['id']] = $ostat2[1];

					}
				}
			}
			else {$ostX[$item['id']] = $ostat[0];}
		}
...
for($z=0;$z<$XX;$z++){
		...
		<tr><td><textarea name='p_koment' rows=5 cols=20 wrap='off' placeholder='Коментарии'></textarea></td>";
...
if($kl[$gr[0]]>0) echo "<p><div style='font-size: 13px;float: left;'>Доставка: &nbsp;&nbsp;&nbsp;</div>".$day[$gr[0]]." $deyname</p>";
if(isset($_SESSION['dostav'])) {if($_SESSION['dostav']<$day[$gr[0]]){$_SESSION['dostav'] = $day[$gr[0]];}}
else {$_SESSION['dostav'] = $day[$gr[0]];}
}

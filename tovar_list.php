...
if($_GET['typecolor']!=''){if($_GET['typecolor']=='0'){$_SESSION['typecolor2']='';}else{$_SESSION['typecolor2']=$_GET['typecolor'];}}
...
if($_GET['search']!=''){if($_GET['search']=='0'){$_SESSION['search2']='';}else{$_SESSION['search2']=$_GET['search'];}}
...
if($_SESSION['typecolor2'] AND $_SESSION['typecolor2'] != $filearray[15]){$OK='';}
...
if($_SESSION['search2']){
if (strtolower_ru(mb_substr($filearray[0], 0, mb_strlen($_SESSION['search2'], 'UTF-8'), "UTF-8")) == strtolower_ru($_SESSION['search2'])) {
...
if($file_array[4]){$info_tc_shop .= '<tr><td>Страна &nbsp;&nbsp;</td><td><b>'.$strana_pok.'</b></td></tr>';}
...
$plus1 = explode(',',$file_array[1]);
$plus2 = explode('-',$plus1[0]);

if (isset($plus2[1])) { $plus = $plus2[1];}
else {$plus = 0;}
//$plus="0.40";
if($file_array[13] == 'usd'){$sm = number_format(($file_array[2] * $settings[12]), 0, '.', ',');$plus = number_format(($plus * $settings[12]), 0, '.', ',');}
elseif($file_array[13] == 'eur'){$sm = number_format(($file_array[2] * $settings[13]), 0, '.', ',');$plus = number_format(($plus * $settings[13]), 0, '.', ',');}
elseif($file_array[13] == 'uan'){$sm = number_format(($file_array[2] * $settings[14]), 0, '.', ',');$plus = number_format(($plus * $settings[14]), 0, '.', ',');}
...
if($file_array[2]){$info_tc_shop .= '<tr><td>Цена &nbsp;&nbsp;</td><td><b>'.round($sm).'</b> р.</td></tr>';}
...
echo '<tr onclick="tovar_view('.$i.','.$dir.',&quot;'.$strana_pok.'&quot;);">
        <td><img src="./gallery/otdel'.$i.'/m/'.$dir.$img.'"/></td>
        <td><b>'.$strana_pok.'</b></td>
        <td><b>'.$file_array[0].'</b></td>
        <td><b>'.$file_array[11].'</b></td>';
echo '<td><b>'.round($sm).'</b> р.</td>';
echo '<td><b>'.$file_array[10].'</b></td></tr>';
}
else {
echo '<li><img onclick="tovar_view('.$i.','.$dir.',&quot;'.$strana_pok.'&quot;);" src="./gallery/otdel'.$i.'/m/'.$dir.$img.'" title="'.$info_tc_shop.'"/></li>';
}
}
}
else{

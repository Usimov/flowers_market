...
$arr = explode('@',$arr);
//tovar_view(23,1441313230,\"Эквадор\");
$keyarr = array_search('tovar_view('.$_GET['dir'].','.$drq[$dri].',\"'.$_GET['stran'].'\");', $arr);
if($arr[$keyarr-1]){$colive .= '<div onclick='.str_replace("\\","",$arr[$keyarr-1]).' id="prev"></div>';}
if($arr[$keyarr+1]){$colive .= '<div onclick='.str_replace("\\","",$arr[$keyarr+1]).' id="next"></div>';}
...
//else{
if($kesh['ostat'][$raz[0]] > 0 OR $kesh['ostat']['null'] > 0){
$colive .= "<option value='".$razmer[$q]."'>".$raz[0]." см.</option>";
}
else{
$colive .= "<option value='".$razmer[$q]."'>".$raz[0]." см.</option>";
}
...
if($file_array[9]){
    $colive .= "<tr><td class='fs21'>Вес</td><td style='text-align: center;'>";
            $colive .= "<input name='ves' type='hidden' value=''>";
        $colive .= $file_array[9]." г.<input name='ves1' type='hidden' value=''>";
    ...
    $ost=explode (",", $file_array[12]);
        if($ost[0]!=''){
            for($v=0;$v<count($ost);$v++){
                ...
                if($ostat[1] > 0){
                    if($kol[$q] <= $ostat[1]){
                        $colive .= "<option value='".str_replace(",","",$kol[$q])."' class='".str_replace(",","",$ostat[0]).'-'.str_replace(",","",$kesh['razmer'][$ostat[0]])."'>".str_replace(",","",$kol[$q])." уп.</option>";
                    }
                }
                else{
                    $colive .= "<option value='".str_replace(",","",$kol[$q])."' class='".str_replace(",","",$ostat[0]).'-'.str_replace(",","",$kesh['razmer'][$ostat[0]])."'>".str_replace(",","",$kol[$q])." уп.</option>";
                ...
                $colive .= "<option value='".str_replace(",","",$kol[$q])."'>".str_replace(",","",$kol[$q])." уп.</option>";
...
if($ostat[0]!=''){
    for($v=0;$v<count($ostat);$v++){
        ...
        if($kesh['raz'][0] != $ostatok[0]){$class = ' style="display:none" class="ost"';}
        else{$class = ' style="display:block" class="ost"';}
        if($ostatok[1] == 0){$ost .= '<div id="x'.md5($ostatok[0].'-'.$kesh['razmer'][$ostatok[0]]).'"'.$class.'>На заказ</div>';}
        else{$ost .= '<div id="x'.md5($ostatok[0].'-'.$kesh['razmer'][$ostatok[0]]).'"'.$class.'>Остаток: '.$ostatok[1].' уп.</div>';}
    }
}
else{
    $ost = 'На заказ';
...
if($file_array[12] == 0){echo 'ostatok["undefined"]=0;';}else{echo 'ostatok["undefined"]='.str_replace(",","",$file_array[12]).';';}
}
?>
...
var cc = "Цена:";
$('#summ').html(<?if($val1){?>+(cen*<?=$val3?>).toFixed(0)+'р.'<?}?>);
$('#summa').html(<?if($val1){?>+(cena*<?=$val3?>).toFixed(0)+'р.'<?}?>);
$('#summ').html('Цена: '+$('#summ').html());
$('#summa').html('Цена: '+$('#summa').html());

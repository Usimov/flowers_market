...
if(($key == 0 OR $key == 2) AND $jcartItemId[3] == 1){
$jcartItemUrl[0] = $jcartItemUrl[0] / 100;
if($jcartItemUrl[0] == 1){$jcartItemUrl[0] = 2;}
$jcartItemPrice = $_POST['jcartItemPrice'][$i] / $jcartItemUrl[0];
}
else{$jcartItemPrice = $_POST['jcartItemPrice'][$i];}
...
$HTML .= "<tr style='height:9.75pt'>
  ...
  style='font-size:10.0pt;'>".$val1.number_format($jcartItemPrice*$val3, $tochka, '.', ',').$val2."</span></p>
  </td>";
if($key == 2){
...
$HTML .= "<td width=60 valign=top style='width:45.0pt;border-top:none;border-left:none;
  ...
  padding:0cm 5.4pt 0cm 5.4pt;height:9.75pt' align=\"right\">
  <p class=MsoNormal align=right style='text-align:right;text-autospace:none'><span
  style='font-size:10.0pt;'>".$val1.number_format($summ, $tochka, '.', ',').$val2."</span></p>
  </td>
 </tr>";
}
...
if($key == 2){
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A'.$row, $r+1)
	                              ...
	                              ->setCellValue('F'.$row, $dataRow['summ']);
}
$bRow = $row;
}
...
$HTML .= $val1.number_format($sm[$jca], $tochka, '.', ',').$val2;
$sm2 = $sm[$jca];
}
else{
$HTML .= $val1.number_format($sm[$jca], $tochka, '.', ',').$val2;
...
$objPHPExcel->getActiveSheet()->setCellValue($cell.$bRow, $val1.number_format($sm[$jca], $tochka, '.', ',').$val2);
$HTML .= "</span></p>
  </td>
 </tr>";
$HTML .= "<tr style='height:10.4pt'>";
...
$objPHPExcel->getActiveSheet()->setCellValue($cell.$bRow, $val1.number_format($sm2, $tochka, '.', ',').$val2);
$HTML .= "<tr style='height:10.4pt'>
  <td width=624 colspan=$colspan valign=top style='width:468.0pt;border:solid windowtext 1.0pt;
  border-top:none;padding:0cm 5.4pt 0cm 5.4pt;height:10.4pt' align=\"right\">
  <p class=MsoNormal align=right style='text-align:right;text-autospace:none'><span
  ...
  padding:0cm 5.4pt 0cm 5.4pt;height:10.4pt' align=\"right\">
  <p class=MsoNormal align=right style='text-align:right;text-autospace:none'><span
  style='font-size:10.0pt;'>".$val1.number_format($sm2, $tochka, '.', ',').$val2."</span></p>
  </td>
 </tr>";
} }
$HTML .= "</table>
<p class=MsoNormal style='text-autospace:none'><span style='font-size:10.0pt;'>&nbsp;</span></p>";
$HTML .= "<p class=MsoNormal style='text-autospace:none'><span style='font-size:10.0pt;'>Итого: ".number_format($sm2, 0, '.', ',')." р. (".num2str($sm2).")</span></p>";
$bRow++;$bRow++;
$objPHPExcel->getActiveSheet()->setCellValue('A'.$bRow, "Итого: ".number_format($sm2, 0, '.', ',')." р. (".num2str($sm2).")");

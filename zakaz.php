...
for($i=0;$i<count($_POST['jcartItemId']);$i++){
  ...
  for($xzx=0;$xzx<count($filearray);$xzx++){
    $filearray[$xzx] = trim($filearray[$xzx]);
  }
...
$ost1=explode ("-", $ost[0]);
          if($ost1[1]!=''){
            if ($ost1[1] <= 0) {$oss=0;}
            else{$oss = $ost1[1] - $_POST['jcartItemQty'][$i];}
            $filearray[12] = $ost1[0]."-".$oss;
          }
}
...
if($ostat[1]){
  ...
  else{$a = explode("-",$ostat[0]);$ostat[0]=$a[1];}
...
$mm = date("m");
$year = date("y");
if(((date('m')==2 or date('m')==4 or date('m')==6 or date('m')==9 or date('m')==11) and $dayday > 30) or $dayday > 31){
    if(date('m') == 2 and $dayday > 28){$dayday = '01';$mm = date('m')+1;}
    $dayday = '01';$mm = date('m')+1;
    if($mm>12){$year = $year +1;$mm = '01';}
}

if ($key == 0 or $key == 1){$p_times = $_SESSION['dost1day'];}
else {$p_times = $_POST['p_times'];}
...
$bRow++;$bRow++;
if($key == 1 or $key==3){$itex3 = '';}else{$itex3 = number_format($sm2, 0, '.', ',')." р. (".num2str($sm2).")";}
$objPHPExcel->getActiveSheet()->setCellValue('A'.$bRow, "Итого:");
$objPHPExcel->getActiveSheet()->setCellValue($cell.$bRow, "       ".$itex3);
